...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Fixed header logic: add class on load
    const header = document.getElementById("header");
    if (header) {
        header.classList.add("fixed-header");
    }

    // Optional: Add shadow effect only after scroll
    ...
    // cookie logic for welcome message

const welcomeDiv = document.getElementById("welcome-message");
const userNameSpan = document.getElementById("userNameSpan");
const resetCheckbox = document.getElementById("resetUserCheckbox");
...
// save cookie on form submit (if first name exists)
form?.addEventListener("submit", function () {
    ...
    if (firstNameInput && firstNameInput.value.trim() !== "") {
        ...
        if (rememberMe && rememberMe.checked) {
            ...
            const now = new Date();
            const expires = new Date(now.getTime() + 48 * 60 * 60 * 1000); // 48 hrs in ms
            document.cookie = `firstName=${encodeURIComponent(name)}; expires=${expires.toUTCString()}; path=/`;
        } else {
